...
//js lint declarations (to avoid error/warnings):

var imageviewerClientGlobals = {
	boundingBox: {x: 0, y: 0, w: 0, h: 0}
};

var imageviewerClientInitialScaleFactor = 1.0;
var imageviewerClientCurrentScaleFactor = 1.0;
...
(function ($) {
    ...
    console.log(opts);
		...
		$canvas.attr("width", _self.width());
		$canvas.attr("height", _self.height());
		...
		function initMinimap() {
			if (minimap && minimapOptions) {
				...
				minimap.html("");
					...
					if(!minimapOptions.image) {
						minimapOptions = $.extend(minimapOptions, {image: img});
					}
					minimap.minimap(minimapScale, fullImageDims.h * minimapScale, minimapOptions);
		...
		function onImageError() {
			...
			var errorMessage = $("<div class='ErrorMessage'>"),
				tryAgain = $("<a>")
		...
		function onCanvasImageLoad(image) {
			...
			if (opts.allowYCentering) {
				y += Math.floor((canvas2DContext.height - image.height) / 2);
			}
			storedWidth = image.width;
			canvas2DContext.drawImage(image, x, y);
			$(".progress").hide();
			if (imageviewerClientCurrentScaleFactor === imageviewerClientInitialScaleFactor && !$("body").hasClass("is-fullscreen")) {
				$("button.object-view__button-icon-favourites").show();
			} else {
				$("button.object-view__button-icon-favourites").hide();
			}
		}
		...
		function recalculateMinScale() {
			if(fullImageDims){
				if ($("#canvas").width() > $("#canvas").height() && params.r === 0 || params.r === 180) {
					minScale = opts.minScale || _self.height() / fullImageDims.h;
				} else {
					minScale = opts.minScale || _self.width() / fullImageDims.w / 1.17;
				}
			}
		}
...
//				.error(onImageError)
		...
		function centerParams(orig) {
			...
			if (fullImageDims.w * orig.s < _self.width() - 1) {
				...
				p.x -= parseInt(diff / 2, 10);
			}
			if (opts.allowYCentering && fullImageDims.h * orig.s < _self.height() - 1) {
				diff = _self.height() - fullImageDims.h * orig.s;
				p.y -= parseInt(diff / 2, 10);
			}
			return p;
		}
		...
		function scaleCanvas(commit) {
			...
			if (fullImageDims) {
				...
				if (previewImage) {
					...
					var w = parseInt(fullImageDims.w * params.s, 10);
					var h = parseInt(fullImageDims.h * params.s, 10);

					$("button.object-view__button-icon-favourites").css(
						{
							top : -p.y + h - 40,
							left : -p.x
						}
					...
					canvas2DContext.drawImage(previewImage.get(0), -p.x, -p.y, w, h);
					imageviewerClientGlobals.boundingBox = {
						x: -p.x,
						y: -p.y,
						w: parseInt(fullImageDims.w * params.s, 10),
						h: parseInt(fullImageDims.h * params.s, 10)
					};

					$canvas.attr("swipable", $canvas.attr("width") >= imageviewerClientGlobals.boundingBox.w);
				}
		...
		this.bind("scaleBy", function (e, factor, commit, center, scalingTo, options) {
			...
			imageviewerClientCurrentScaleFactor = params.s;
		});
		...
		this.bind("scaleToFullHeightOr1s", function(e) {
			if (fullImageDims && fullImageDims.h) {
				params.y = 0;
				params.s = _self.height() / fullImageDims.h;
				if(params.s > 1.0) { params.s = 1.0; }
				scaleCanvas(true);
			}
		});

		this.bind("scaleToFullPage", function (e, canvasMaximumHeight, canvasMaximumWidth) {
			if(!fullImageDims){
				return;
			}

			if (typeof canvasMaximumHeight === 'undefined') {
				canvasMaximumHeight = opts.params.canvasMaximumHeight;
				canvasMaximumWidth = opts.params.canvasMaximumWidth;
			}

			if (canvasMaximumWidth / canvasMaximumHeight < fullImageDims.w / fullImageDims.h) {
				params.s = parseFloat(canvasMaximumWidth / fullImageDims.w);
			} else {
				params.s = parseFloat(canvasMaximumHeight / fullImageDims.h);
			}
			...
			imageviewerClientInitialScaleFactor = params.s;
			imageviewerClientCurrentScaleFactor = params.s;
			scaleCanvas(true);
		});
	...
	$.fn.imageViewerHandlers = function (opts) {
		...
		if (opts && opts.mousewheel) {
			this.mousewheel(function (e, which) {
				if(opts.zoomWithinBounds && (
					e.clientX - $(this).position().left < imageviewerClientGlobals.boundingBox.x ||
					e.clientX - $(this).position().left > imageviewerClientGlobals.boundingBox.x + imageviewerClientGlobals.boundingBox.w ||
					e.clientY - $(this).position().top < imageviewerClientGlobals.boundingBox.y ||
					e.clientY - $(this).position().top > imageviewerClientGlobals.boundingBox.y + imageviewerClientGlobals.boundingBox.h
				)) {
					return true;
				}
				e.preventDefault();
				if (_scrollLock) {
					...
					if (which < 0) {
						if (typeof opts.initialScaleFactor !== "undefined") {
							if (imageviewerClientCurrentScaleFactor * 0.9 > imageviewerClientInitialScaleFactor) {
								$(this).trigger('scaleBy', [0.9, false]);
							} else {
								$(this).trigger('scaleBy', [imageviewerClientInitialScaleFactor, false, null, true]);
							}
						} else {
							$(this).trigger('scaleBy', [0.9, false]);
						}
...
(function ($) {
	...
	$.fn.imageviewerTouchHandlers = function (opts) {
		...
		$(this).on("touchstart", function (e) {
			//== canvas blocks body click even on mobile
			$("body").trigger("click");
			...
			if (touches.length === 1) {
				...
				if (imageviewerClientGlobals.boundingBox.x < touchmap.startPos.x && touchmap.startPos.x < imageviewerClientGlobals.boundingBox.x + imageviewerClientGlobals.boundingBox.w && imageviewerClientGlobals.boundingBox.y < touchmap.startPos.y && touchmap.startPos.y < imageviewerClientGlobals.boundingBox.y + imageviewerClientGlobals.boundingBox.h) {
					canvasDrawingAreaControl = true;
				} else {
					canvasDrawingAreaControl = false;
					return true;
				}
			}
		...
		$(this).on("touchend", function (e) {
			...
			var timeDifference = new Date().getTime() - touchmap.tapStart;

			if (timeDifference < 400 && typeof touchmap.positions[0] === "undefined") {
				...
				if (typeof opts.onswipeleft === "function" && typeof opts.onswiperight === "function") {
					box = getBox();
					touchendX = touchmap.positions[0].x - box.x0;
					deltaX = touchmap.startPos.x - touchendX;
			...
			if (_state === 'default') {
				if (touches.length === 2) {
					...
					if (touchmap.pinchDelta < 20 && touchmap.pinchDelta > -20) {
						...
						if (typeof opts.initialScaleFactor !== "undefined") {
							if (imageviewerClientCurrentScaleFactor * sHeur > imageviewerClientInitialScaleFactor) {
		...
		if (typeof MSGesture === "function") {
			...
			if (typeof opts.ontap === "function") {
				canvas.addEventListener("MSGestureTap", function (event) {
					opts.ontap();
				});
			}

			canvas.addEventListener("MSGestureChange", function (event) {
				if (event.detail === event.MSGESTURE_FLAG_NONE && event.scale !== 1) {
					if (typeof opts.initialScaleFactor !== "undefined") {
						if (imageviewerClientCurrentScaleFactor * event.scale > imageviewerClientInitialScaleFactor) {
							$(this).trigger("scaleBy", [event.scale, false]);
						} else {
							$(this).trigger("scaleBy", [imageviewerClientInitialScaleFactor, false, null, true]);
						}
					} else {
				...
				} else if (event.detail === event.MSGESTURE_FLAG_NONE && event.translationY !== 0 && !(imageviewerClientGlobals.boundingBox.x < event.offsetX && event.offsetX < imageviewerClientGlobals.boundingBox.x + imageviewerClientGlobals.boundingBox.w && imageviewerClientGlobals.boundingBox.y < event.offsetY && event.offsetY < imageviewerClientGlobals.boundingBox.y + imageviewerClientGlobals.boundingBox.h)) {
					document.documentElement.scrollTop = document.documentElement.scrollTop - event.translationY;
				}
			});
			...
			if (typeof opts.onswipeleft === "function" || typeof opts.onswiperight === "function") {
				...
				canvas.addEventListener("MSGestureStart", function (event) {
					translation = event.translationX;
				});

				canvas.addEventListener("MSGestureEnd", function (event) {
					if (translation > 3) {
						gesture.stop();
						opts.onswiperight();
...
(function ($) {
	...
	$.fn.minimap = function (scale, height, options) {
		...
		if (opts.image) {
			_self.css({
				backgroundImage: "url(" + opts.image.replace("(", "%28").replace(")", "%29") + ")"
			});
		}
